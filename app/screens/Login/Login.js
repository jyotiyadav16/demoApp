...
const Login = ({ navigation }) => {
  ...
  const onPressLogin = () => {
    if (checkValidation()) {
      ...
      dispatch(fetchLogin(payload)).then(unwrapResult).then((res) => {
        if (res.status == 200) {
          ...
          showToast('success', 'Login sucessfull', 2500);
        }
      }).catch((error) => {
        console.log({ error })
        showToast('danger', 'Failed to login ', 2500);
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={() => navigation.navigate('RegisterScreen')}>
        <Text style={styles.forgot_button}> Doesn't have an account yet? Sign Up</Text>
      </TouchableOpacity>
